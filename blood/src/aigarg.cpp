...
static void playStatueBreakSnd(SPRITE *, XSPRITE *);
...
static void playStatueBreakSnd(SPRITE *pSprite, XSPRITE *pXSprite)
{
    aiPlay3DSound(pSprite, 313, AI_SFX_PRIORITY_1);
}
    ...
    for (short nSprite2 = headspritestat[6]; nSprite2 >= 0; nSprite2 = nextspritestat[nSprite2])
    {
        ...
        if (cansee(x, y, z, pSprite->sectnum, x2, y2, z2, pSprite2->sectnum))
        {
            ...
            if (dzCenter > -0x333)
                ...
                aim.dz = divscale10(dzCenter, nDist)+9460;
            else if (dzCenter < -0xb33 && dzCenter > -0x3000)
                aim.dz = divscale10(dzCenter, nDist)+9460;
            else if (dzCenter < -0x3000)
                aim.dz = divscale10(dzCenter, nDist)-7500;
            else
                aim.dz = divscale10(dzCenter, nDist);
        }
        else

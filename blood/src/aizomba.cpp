...
static void HackSeqCallback(int, int nXSprite)
{
    ...
    actFireVector(pSprite, 0, 0, dx, dy, dz, kVectorZombieAxe);
}
...
{
    dassert(pSprite->type >= kDudeBase && pSprite->type < kDudeMax, 163);
    DUDEINFO *pDudeInfo = &dudeInfo[pSprite->type - kDudeBase];
    int dx = pXSprite->at20_0-pSprite->x;
    int dy = pXSprite->at24_0-pSprite->y;
    int nAngle = getangle(dx, dy);
...
{
    dassert(pSprite->type >= kDudeBase && pSprite->type < kDudeMax, 357);
    DUDEINFO *pDudeInfo = &dudeInfo[pSprite->type-kDudeBase];

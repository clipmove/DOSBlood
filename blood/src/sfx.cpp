...
void sfxKillAllSounds(SPRITE* pSprite)
{
    if (FXDevice == -1)
        return;
    for (int i = nBonkles - 1; i >= 0; i--)
    {
        BONKLE *pBonkle = BonkleCache[i];
        if (pSprite && pBonkle->at10 != pSprite)
            continue;
        if (pBonkle->at0 > 0)
        {
            FX_EndLooping(pBonkle->at0);
...
{
    const int dx = mulscale30(Cos(gMe->pSprite->ang+512), 43);
    const int dy = mulscale30(Sin(gMe->pSprite->ang+512), 43);
    earL0 = earL;
    earR0 = earR;

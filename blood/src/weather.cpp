...
void CWeather::Draw(char *pBuffer, long nX, long nY, long nZ, int nAng, int nHoriz, short nSector, long nClock, int nInterpolate, unsigned int uMapCRC)
{
    if (!Status() || !pBuffer)
        return;
    ...
    else if (!nCountLimited && (nWeatherForecast != WEATHERTYPE_NONE)) // fade out complete, now switch to new weather
    {
        ...
        memset(clipbit, 0, ((nLimit*2)+7)>>3); // only clear bits we're using
    }
}
...
void CWeather::Process(long nX, long nY, long nZ, int nAng, short nSector, char bSpinning, long nTime, int nClipDist, unsigned int uMapCRC)
{
